...
def get_exchange_rate(api_key, from_currency, to_currency):
    ...
    if response.status_code != 200:
        print(f"{Fore.RED}❌ Failed to get the exchange rate.")
        return None
    
    exchange_rate = data['conversion_rates'].get(to_currency)
    if exchange_rate is None:
        print(f"{Fore.RED}❌ Currency code {to_currency} is not supported.")
...
def list_currency_codes(api_key):
    ...
    if response.status_code != 200:
        print(f"{Fore.RED}❌ Failed to retrieve currency codes.")
        return []
...
def main():
    api_key = '3781e415849b758cc211149a'  # Replace with your actual API key
    ...
    while True:
        ...
        if choice == '1':
            ...
            if exchange_rate:
                try:
                    amount = float(input(f"{Fore.CYAN}Enter the amount in {from_currency}: {Style.RESET_ALL}"))
